...
editor="nano"
format="md"
jekyllpath="/var/www/dcode/_posts"
...
echo $varpath/$filename

touch $varpath/$filename

# Add initial YAML to the top of the new bit
echo "---" >> $jekyllpath/$filename
echo "layout: ${layout}" >> $jekyllpath/$filename
echo "title: \"${title}\"" >> $jekyllpath/$filename
echo "category:" >> $jekyllpath/$filename
echo "---" >> $jekyllpath/$filename
echo "" >> $jekyllpath/$filename
